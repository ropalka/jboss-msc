 ...
 * ({@link StartContext#asynchronous()}/{@link StopContext#asynchronous()}) and do one of the following:
 *
 * <ul>
 * <li>Initiate your task in start()/stop(), and utilize a callback (NIO, ThreadPoolExecutor.terminated(), etc.) to call
 * ({@link StartContext#complete()}/{@link StopContext#complete()}) when your start/stop completes instead of blocking</li>
 * <li>Delegate your blocking task to a thread pool ({@code Executor}) which calls
 * ({@link StartContext#complete()}/{@link StopContext#complete()}) when done</li>
